    ...
    public function getResults($simplify = TRUE)
    {
        if (!$this->isProcessed) {
            ...
            for ($i = 0; $i < 100; $i++) {
                ...
                if (empty($body)) {
                    if ($i == 0) {
                        throw new EmptyBodyException('Unable to read HTML body for scraping (empty)', 1480766267);
                    } else {
                        break;
                    }
                }
            ...
            foreach ($urls as $url) {
                ...
                if (is_int(strpos($url, 'r.bat.bing.com'))) continue;
